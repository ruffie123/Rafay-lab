/* ============================================
   ...
   Telegram + WhatsApp + 5 Bot Types + AI
   ============================================ */
...
(function () {
    ...
    const TG = "# RafayLab Telegram Bot\n# 1. pip install python-telegram-bot\n# 2. nano bot.py\n# 3. paste this code\n# 4. save: Ctrl+O Enter Ctrl+X\n# 5. python3 bot.py\n\n";
    const WA = "# RafayLab WhatsApp Bot\n# 1. npm init -y\n# 2. npm install whatsapp-web.js qrcode-terminal\n# 3. save as bot.js\n# 4. node bot.js\n# 5. Scan QR\n\n";
    ...
    T.faq = function (n, cred, w) {
        if (platform === 'whatsapp') {
            return waT(n, cred, w, '    const faq = {"price": "From PKR 100", "timing": "9 AM - 11 PM", "location": "Lahore"};\n    let r = "Ask: price, timing, location";\n    for (const k in faq) { if (msg.body.toLowerCase().includes(k)) r = faq[k]; }\n    await msg.reply(r);\n');
        }
    ...
    T.custom = function (n, cred, w, prompt, features) {
        const f = features || 'general';
        if (platform === 'whatsapp') {
            return waT(n, cred, w, '    // Requirement: ' + prompt + '\n    // Features: ' + f + '\n    if (msg.body === "/start") {\n        await msg.reply(WELCOME + "\\nBot: " + BOT_NAME);\n    } else {\n        await msg.reply("Got: " + msg.body);\n    }\n');
        }
    ...
    if (gb) {
        gb.addEventListener('click', function () {
            ...
            if (selectedType === 'custom') {
                ...
                code = T.custom(name, cred, welcome, p, f);
            } else {
                code = T[selectedType](name, cred, welcome);
            }
            co.textContent = code;
